...
static struct stc311x_chip * chip= NULL;
...
static void STC311x_ForceVM(void)
{
  ...
  value=STC31xx_ReadByte(STC311x_REG_MODE);
  STC31xx_WriteByte(STC311x_REG_MODE,value | STC311x_FORCE_VM);   
}
*/
...
//Check battery presence
  ...
  else
  {
	...
	//Lately fully compensation
      if(BattData.BattState > BATT_IDLE && BattData.SOC >= 990 && BattData.SOC < 995 && BattData.AvgCurrent > 170)
      {
        BattData.SOC = 990;
...
static void stc311x_work(struct work_struct *work)
{
    ...
    {
        /* results available */
        chip->batt_soc = GasGaugeData.SOC+5;
...
void stc311x_QuickStart(struct i2c_client *client)
{
			     ...
			     union power_supply_propval *val)
{
	if(chip==NULL)
		{
			printk("sec_hal_fg_get_property return for error ");
			return false;
		}
	switch (psp) {
