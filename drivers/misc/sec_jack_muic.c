...
static void sec_jack_set_type(struct sec_jack_info *hi, int jack_type)
{
	...
	if (jack_type == hi->cur_jack_type) {
		pr_info(MODULE_NAME "%s return, same type reason %d\n",
			__func__, jack_type);
		return;
	}
			...
			if (adc <= zones[i].adc_high) {
				if (++count[i] > zones[i].check_count) {
					...
					if (recheck_jack == true && i == 3) {
						pr_info(MODULE_NAME "something wrong connectoin!\n");
						handle_jack_not_inserted(hi);
						recheck_jack = false;
						return;
					}
...
void earkey_status_change(int pressed, int kcode)
{
	...
	if (pressed) {
		msleep(50);
		input_report_key(hi->input, kcode, 1);
		switch_set_state(&switch_sendend, 1);
		input_sync(hi->input);
		send_end_pressed = 1;
	} else {
		input_report_key(hi->input, kcode, 0);
		switch_set_state(&switch_sendend, 0);
		input_sync(hi->input);
		send_end_pressed = 0;
	}
...
static void sec_jack_det_work_func(struct work_struct *work)
{
	...
	while (time_left_ms > 0) {
		...
		if (!jack_state) {
			pr_info("%s : err det jack state %d\n",
				__func__, jack_state);
			/* jack not detected. */
			handle_jack_not_inserted(hi);
			return;
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(audio))
		pr_info("Failed to create class(audio)!\n");

	earjack = device_create(audio, NULL, 0, NULL, "earjack");
	if (IS_ERR(earjack))
		pr_info("Failed to create device(earjack)!\n");

	ret = device_create_file(earjack, &dev_attr_key_state);
	if (ret)
		pr_info("Failed to create device file in sysfs entries(%s)!\n",
...
/*
	...
	if (ret) {
		pr_info("%s : Failed to request_irq.\n", __func__);
	...
	if (ret) {
		pr_info("%s : Failed to enable_irq_wake.\n", __func__);
