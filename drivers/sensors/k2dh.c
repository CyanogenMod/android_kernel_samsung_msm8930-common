...
#include "k2dh_reg.h"
#ifdef CONFIG_SENSOR_K2DH_INPUTDEV
				...
				struct k2dh_acc *acc)
{
	...
	if (data->axis_adjust)
		k2dh_xyz_position_adjust(acc, data->position);
	return 0;
}
...
{
	k2dh_read_accel_xyz(g_k2dh, &g_k2dh->acc_xyz);
	pr_debug("%s: x: %d, y: %d, z: %d\n", __func__,
		g_k2dh->acc_xyz.x, g_k2dh->acc_xyz.y, g_k2dh->acc_xyz.z);
	input_report_abs(g_k2dh->input, ABS_X, g_k2dh->acc_xyz.x);
	input_report_abs(g_k2dh->input, ABS_Y, g_k2dh->acc_xyz.y);
	input_report_abs(g_k2dh->input, ABS_Z, g_k2dh->acc_xyz.z);
	input_sync(g_k2dh->input);
	schedule_delayed_work(&g_k2dh->work, msecs_to_jiffies(
		atomic_read(&g_k2dh->delay)));
}

/* ----------------- *
   Input device interface
 * ------------------ */
static int k2dh_input_init(struct k2dh_data *data)
{
		       ...
		       const struct i2c_device_id *id)
{
	...
	if (err) {
		pr_err("%s: cound not make k2dh sensor symlink(%d).\n",
			__func__, err);
		goto err_sensors_initialize_symlink_k2dh;
	}
#endif
...
#ifdef CONFIG_SENSOR_USE_SYMLINK
err_sensors_initialize_symlink_k2dh:
