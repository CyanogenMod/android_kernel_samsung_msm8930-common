					...
					unsigned long count, void *data)
{
    ...
    BT_DBG("bluesleep_write_proc_btwrite (%c) ", b);
...
static int bluesleep_probe(struct platform_device *pdev)
{
	...
	BT_DBG("bluesleep_probe ext_wake [%d]", bsi->has_ext_wake);

	if (bsi->has_ext_wake) {
		bsi->ext_wake = res->start;

		BT_DBG("bluesleep_probe ext_wake");
        ...
        // PMIC GPIO
		ret = pm8xxx_gpio_config(bsi->ext_wake, &bt_wake);
		if (ret<0) {
		...
		BT_DBG("bluesleep_probe ext_wake direction out");
	} else
		set_bit(BT_EXT_WAKE, &flags);
