...
static struct mipi_dsi_phy_ctrl dsi_video_mode_phy_db ={
	...
	/* strength */ 
		{0xee, 0x02, 0x86, 0x00},
	/* pll control */ /* Ref to the calculation resule in kernel*/
...
static int mipi_video_hx8369b_qhd_pt_init(void)
{
	int ret;

	printk(KERN_ERR "mipi_video_hx8369b_qhd_pt_init\n");
	...
	pinfo.wait_cycle = 0;
	pinfo.bpp = 24;
	pinfo.lcdc.h_back_porch  = 96;	/* 64 */
	pinfo.lcdc.h_front_porch = 106;	/* 64 */
	pinfo.lcdc.h_pulse_width =  60;
	pinfo.lcdc.v_back_porch  = 12;	/* 4 */
	pinfo.lcdc.v_front_porch =  8;
	pinfo.lcdc.v_pulse_width = 4;	/* 2 */
	...
	printk("[LCD] mipi HBP=%d, HFP=%d, HPW=%d, VBP=%d, VFP=%d, VPW=%d\n",
...
module_init(mipi_video_hx8369b_qhd_pt_init);
