...
#define GPIO_WL_HOST_WAKE	72
#define GPIO_WL_REG_ON		PM8917_GPIO_PM_TO_SYS(30)
...
static int brcm_wlan_power(int onoff)
{
	...
	if (onoff) {
		if (gpio_request(GPIO_WL_HOST_WAKE, "wlan_host_wake" )) {
			printk("%s: gpio request wlan_host_wake pin failed\n", __func__);
		}

		rc = gpio_request(GPIO_WL_REG_ON, "wlan_en");
		if(!rc) {
			struct pm_gpio wlan_en = {
			...
			rc = pm8xxx_gpio_config(GPIO_WL_REG_ON, &wlan_en);
			if(rc) {
				printk("%s:  Failed to configure gpio %d\n",  __func__, GPIO_WL_REG_ON);
			}
			rc = gpio_direction_output(GPIO_WL_REG_ON, GPIO_WLAN_LEVEL_HIGH);
		}
		if (rc) {
			printk("%s: gpio request %d pin failed\n",  __func__, rc);
		}
		msleep(80);
	} else {
		gpio_direction_output(GPIO_WL_REG_ON, GPIO_WLAN_LEVEL_LOW);
