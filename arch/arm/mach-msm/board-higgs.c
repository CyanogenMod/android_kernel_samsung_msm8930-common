...
static void mag_init_gpio(void);
#endif
...
#endif
static struct i2c_board_info geo_i2c_board_info_rev0[] = {
	{
...
static struct i2c_board_info sns_i2c_board_info_rev0[] = {
	{
		I2C_BOARD_INFO("accelerometer", 0x18),
	},
};
static struct i2c_board_info sns_i2c_board_info_rev1[] = {
	{
		I2C_BOARD_INFO("accelerometer", 0x18),
...
static void nfc_simvcc(bool onoff)
{
	int ret;
	static struct regulator *reg_l6;
	if (!reg_l6) {
		reg_l6 = regulator_get(NULL, "8917_l6");
		if (IS_ERR(reg_l6)) {
			pr_err("could not get 8917_l6, rc = %ld=n",
				PTR_ERR(reg_l6));
			return;
		}
		ret = regulator_set_voltage(reg_l6, 2950000, 2950000);
		if (ret) {
			pr_err("%s: unable to set reg_l6 voltage to 2.95V\n",
				__func__);
			return;
		}
	}
	if (onoff) {
		if (!regulator_is_enabled(reg_l6)) {
			...
			if (ret) {
				pr_err("enable reg_l6 failed, rc=%d\n", ret);
				return;
			}
			pr_info("NFC VCC 2.95V on is finished.\n");
		} else
			pr_info("NFC VCC 2.95V is already on.\n");
	} else {
		if (regulator_is_enabled(reg_l6)) {
			ret = regulator_disable(reg_l6);
		if (ret) {
			pr_err("disable reg_l6 failed, rc=%d\n", ret);
			return;
		}
		pr_info("NFC VCC 2.95V off is finished.\n");
		} else
			pr_info("NFC VCC 2.95V is already off.\n");
	}
}
#endif
#if defined(CONFIG_NFC_PN547) || defined(CONFIG_BCM2079X_NFC_I2C)
static void nfc_conf_gpio(void)
{
	pr_debug("nfc_conf_gpio\n");
...
#ifdef CONFIG_INPUT_YAS_SENSORS
static struct platform_device *yas_magnetic_devices[] __initdata = {
	&geo_i2c_gpio_device,
};
#endif
#if defined(CONFIG_NFC_PN547)
static struct platform_device *nfc_pn544_device[] __initdata = {
	&pn544_i2c_gpio_device,
};
#endif
#if defined(CONFIG_BCM2079X_NFC_I2C)
static struct platform_device *nfc_bcm2079x_device[] __initdata = {
	&bcm2079x_i2c_gpio_device,
};
#endif
static struct platform_device *higgs_devices[] __initdata = {
	&msm8960_device_otg,
...
#ifdef CONFIG_INPUT_YAS_SENSORS
static struct i2c_registry msm8930_i2c_yas_sensors_rev0[] __initdata = {
	{
		MSM_GEO_I2C_BUS_ID,
		geo_i2c_board_info_rev0,
		ARRAY_SIZE(geo_i2c_board_info_rev0),
	},
	{
		MSM_SNS_I2C_BUS_ID,
		sns_i2c_board_info_rev0,
		ARRAY_SIZE(sns_i2c_board_info_rev0),
	},
};

static struct i2c_registry msm8930_i2c_yas_sensors_rev1[] __initdata = {
	{
		MSM_SNS_I2C_BUS_ID,
		sns_i2c_board_info_rev1,
		ARRAY_SIZE(sns_i2c_board_info_rev1),
	},
};
#endif
#if defined(CONFIG_NFC_PN547)
static struct i2c_registry msm8930_i2c_nfc_pn544[] __initdata = {
	{
		MSM_NFC_I2C_BUS_ID,
		pn544_info,
...
#endif
#if defined(CONFIG_BCM2079X_NFC_I2C)
static struct i2c_registry msm8930_i2c_nfc_bcm2079x[] __initdata = {
	{
		MSM_NFC_I2C_BUS_ID,
		nfc_bcm2079x_info,
...
#ifdef CONFIG_INPUT_YAS_SENSORS
	if (system_rev > BOARD_REV00){
		for (i = 0; i < ARRAY_SIZE(msm8930_i2c_yas_sensors_rev1); ++i) {
			i2c_register_board_info(msm8930_i2c_yas_sensors_rev1[i].bus,
					msm8930_i2c_yas_sensors_rev1[i].info,
					msm8930_i2c_yas_sensors_rev1[i].len);
		}
	}
	else {
		for (i = 0; i < ARRAY_SIZE(msm8930_i2c_yas_sensors_rev0); ++i) {
			i2c_register_board_info(msm8930_i2c_yas_sensors_rev0[i].bus,
					msm8930_i2c_yas_sensors_rev0[i].info,
					msm8930_i2c_yas_sensors_rev0[i].len);
		}
	}
#endif
...
#if defined(CONFIG_NFC_PN547)
		i2c_register_board_info(msm8930_i2c_nfc_pn544[0].bus,
				msm8930_i2c_nfc_pn544[0].info,
				msm8930_i2c_nfc_pn544[0].len);
#endif
#if  defined(CONFIG_BCM2079X_NFC_I2C)
		i2c_register_board_info(msm8930_i2c_nfc_bcm2079x[0].bus,
				msm8930_i2c_nfc_bcm2079x[0].info,
				msm8930_i2c_nfc_bcm2079x[0].len);
#endif
#ifdef CONFIG_MSM_CAMERA
...
void __init msm8930_higgs_init(void)
{
#ifdef CONFIG_SEC_DEBUG
	sec_debug_init();
#endif
...
#ifdef CONFIG_INPUT_YAS_SENSORS
	if (system_rev == BOARD_REV00)
		platform_add_devices(yas_magnetic_devices, ARRAY_SIZE(yas_magnetic_devices));
#endif
#if defined(CONFIG_NFC_PN547)
		platform_add_devices(nfc_pn544_device, ARRAY_SIZE(nfc_pn544_device));
#endif
#if  defined(CONFIG_BCM2079X_NFC_I2C)
		platform_add_devices(nfc_bcm2079x_device, ARRAY_SIZE(nfc_bcm2079x_device));
#endif
...
#endif
	platform_add_devices(higgs_devices, ARRAY_SIZE(higgs_devices));
#ifdef CONFIG_MSM_CAMERA
	msm8930_init_cam();
#endif
	msm8930_init_mmc();
